...
def login():
    with models.get_session("auth") as session:
        user = session.query(models.User).filter(models.User.name == request.json.get("user")).one_or_none()
        password = request.json.get("password", "")

        if auth.verify_password(password, user.password_hash.encode("utf8") if user else None):
...
def add_stats(user):
    with models.get_session("readwrite") as session:
...
def get_stats(user):
    ...
    with models.get_session("read") as session:
        try:
...
def add_user(user):
    ...
    with models.get_session("adduser") as session:
